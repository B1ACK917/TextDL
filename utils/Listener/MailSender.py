...
class Sender(object):
    ...
    def send(self, to_address, subject, content, file_path_list):
        ...
        message['From'] = u'DownLoader <{}>'.format(self._address)
        message['Subject'] = Header(u'{}'.format(subject), 'utf8').encode()
        message['To'] = u'{}'.format(to_address)

        attr2 = MIMEText(content, 'plain', 'utf-8')
        message.attach(attr2)

        for file_path in file_path_list:
            attr = MIMEText(open(r'{}'.format(file_path), 'rb').read(), 'base64', 'utf-8')
            attr["content_Type"] = 'application/octet-stream'
            attr.add_header('Content-Disposition', 'attachment', filename=os.path.split(file_path)[1])
            message.attach(attr)

        self._server.sendmail(self._address, [to_address, self._address], message.as_string())
